...
const CarApplication = require('../models/CarApplicationSchema')
const Dept = require('../models/deptSchema')
const utils = require('../utils/utils')
const log4js = require('../utils/log4js')
const jwt = require('jsonwebtoken')
const config = require('../config')
router.prefix('/car_application')
...
router.get('/list', async function (ctx) {
  try {
    ...
    // 根据条件查询所有用户列表
    const query = CarApplication.find(params)//查询所有数据
    const list = await query.skip(skipIndex).limit(page.pageSize)//根据查出的所有数据截取对应页数的数据
    const total = await CarApplication.countDocuments(params);
...
router.post('/operate', async (ctx)=>{
  ...
  try {
    ...
    if (action == 'create') {
      ...
      let count = await CarApplication.countDocuments()
      orderNo += count
      ...
      // 审批日志
      params.auditLogs = []
      res = await CarApplication.create(params)
      info = '创建成功'
    } else if (action === 'delete') {
      const res = await CarApplication.findByIdAndUpdate(_id, { applyState: 5 })
      info = '作废成功'
    }
...
router.post('/approve', async (ctx)=>{
  ...
  const ueserInfo = utils.decodeToken(token)
  try {
    const doc = await CarApplication.findById(_id)
    if (action === 'refuse') {
      ...
      await CarApplication.findByIdAndUpdate(_id, { applyState: 3, auditLogs})
    } else {
      if (doc.auditLogs.length === doc.auditFlows.length) { // 证明已经审核完了
        ...
        await CarApplication.findByIdAndUpdate(_id, { applyState: 4, auditLogs})
      } else {
        const auditLogs = doc.auditLogs
        auditLogs.push({
          userId: ueserInfo.userId,
        ...
        await CarApplication.findByIdAndUpdate(_id, { applyState: 2, auditLogs, curAuditUserName})
      }
    }
